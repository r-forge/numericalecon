...
To print a regression result from `lm` or `glm`, the method `printReg`
applied to `lm` and `glm` objects generates a latex equation with the
coefficient estimates and their standard errors. To have the equation
printed in Latex format in a R-Markdown document, the chunk option
`results='asis'` must be added. For example, the following is an
estimated wage equation using the `PSID1976` dataset from the `AER`
package (@aer):
...
If we just want to create a regression equation from a formula, the
method admits objects of class `formula`. Here is an example:

```{r, results='asis'}
printReg(log(wage)~education*female+age+I(age^2), maxpl=3)
```
...
The tool also offers the possibility of having an over-identified
model. By default, we have `d=c(100, -1, 2, 0)` and `sigX=c(5,5)`,
which implies that only one exogenous variable is included in the
demand function. By replacing `d[4]` by a non-zero value, the demand
becomes an equation with the exogenous variables $X_1$ and $X_2$, and
`sigX` contains the variance of the two variables:

```{r, results='asis', out.width="50%", fig.align="center"}
sys2 <- genDemSup(n=200, d=c(100, -1, 2, 0.5))
print(sys2)
plot(sys2, nCurve=15)
```

We can test the strength of the instruments using the `testRegF` function:

```{r, results='asis'}
fitu <- lm(P~X1+X2+Z, sys2$dat)
fitr <- lm(P~Z, sys2$dat)
printReg(fitu)
printReg(fitr)
testRegF(fitu, fitr)
```

We therefore have strong instruments. We can also test the exogeneity
of the instruments, by using the `specTest` method of `momentfit` or
the `printReg` method for objects of class `tsls`:

```{r, results='asis'}
fit <- tsls(Q~P+Z, ~X1+X2+Z, data=sys2$dat)
printReg(fit, strength=TRUE, Jtest=TRUE, Exo=TRUE)
```
